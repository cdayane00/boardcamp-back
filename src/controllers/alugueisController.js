...
export default class Alugueis {
    ...
    static postRental = async (req, res) => {
        ...
        try {
            ...
            return res
                .status(500)
                .json({ message: 'Erro ao criar Aluguel', error: err.message });
        }
    };
    ...
    static returnRental = async (req, res) => {
        ...
        try {
            ...
            if (rental.rows.length === 0) {
                return res.status(400).send('Aluguel não existe');
            }

            if (rental.rows[0].returnDate) {
                return res.status(400).send('Aluguel já devolvido');
            }
            ...
            return res.status(500).json({
                message: 'Erro ao devolver aluguel',
                error: err.message,
            });
        }
    };
    ...
    static deleteRental = async (req, res) => {
        ...
        try {
            ...
            if (rental.rows[0].returnDate) {
                return res.status(400).send('Aluguel já retornado');
            }
            ...
            return res
                .status(500)
                .json({ message: 'Erro ao deletar', error: err.message });
        }
    };
